...
export async function sendAccessNotification(accessData) {
    try {
        ...
        // Determinar email destinatario desde variables de entorno
        let recipientEmail;
        if (accessData.accountName === 'Grupo Torres') {
            recipientEmail = config.emails.grupoTorres;
        } else if (accessData.accountName === 'Ivercasa') {
            recipientEmail = config.emails.ivercasa;
        } else {
        ...
        if (!recipientEmail) {
            throw new Error('Email destinatario no configurado en variables de entorno');
        }
        ...
        console.warn('El acceso se registró en Firebase pero el email falló');
